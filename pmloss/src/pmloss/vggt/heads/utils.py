...
def position_grid_to_embed(pos_grid: torch.Tensor, embed_dim: int, omega_0: float = 100.0) -> torch.Tensor:
    ...
    emb_x = make_sincos_pos_embed(embed_dim // 2, pos_flat[:, 0], omega_0=float(omega_0))  # [1, H*W, D/2]
    emb_y = make_sincos_pos_embed(embed_dim // 2, pos_flat[:, 1], omega_0=float(omega_0))  # [1, H*W, D/2]
